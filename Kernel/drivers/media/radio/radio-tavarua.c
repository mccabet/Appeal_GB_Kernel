...
static int tavarua_fops_release(struct file *file)
{
	...
	if (!is_turned_off)	{
		...
		if (retval >= 0) {
			if (!wait_for_completion_timeout(
				...
				retval = tavarua_write_register(radio,
						RDCTRL, FM_RESET);
				FMDERR("reset return = %x"
					"& sending ready evt to APP", retval);
				tavarua_q_event(radio, TAVARUA_EVT_RADIO_READY);
			    atomic_inc(&radio->users);
			}
		}
	...
	/* BEGIN: Modified Code */
	FMDERR("Wait for the READY event to be triggered"
		"before un-registering the Interrupt handler");
	msleep(TAVARUA_DELAY*10);
	/* END: Modified Code */
		...
		struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_PRIVATE_TAVARUA_STATE:
		...
		/* check if off */
			...
			FMDBG("turning off...\n");
			FMDBG("%s, Disable Interrupts\n", __func__);
			/* disable irq */
			retval = tavarua_disable_interrupts(radio);
			if (retval < 0) {
				printk(KERN_ERR "%s: failed to disable"
					"Interrupts\n", __func__);
				return retval;
			}
