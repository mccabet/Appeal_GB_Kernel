...
static int vnet_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (vnet_start_xmit_flag != 0 || pdp_tx_flag == 1) {
		pr_err("[MULTIPDP] <%s> Cannot transmit data packet!! [%d], [%d]\n",
			__func__, vnet_start_xmit_flag, pdp_tx_flag);
			   ...
			   unsigned long arg)
{
	...
	case HN_PDP_TXSTART:
		pdp_tx_flag = 0;
		if (pdp_tx_flag != 0) {
			pr_err("[MULTIPDP] <%s> pdp_tx_flag clear error!\n",
				__func__);
			pdp_tx_flag = 0;
		} else {
			pr_info("[MULTIPDP] <%s> pdp_tx_flag set 0\n", __func__);
		}
		...
		for (slot = 0; slot < MAX_PDP_CONTEXT; slot++) {
			...
			if (dev != NULL && dev->type == DEV_TYPE_NET) {
				...
				pr_err("[MULTIPDP] %s %s netif_wake\n",
	...
	case HN_PDP_TXSTOP:
		for (slot = 0; slot < MAX_PDP_CONTEXT; slot++) {
			...
			if (dev != NULL && dev->type == DEV_TYPE_NET) {
				...
				pr_err("[MULTIPDP] %s %s netif_stop\n",
					__func__, dev->vn_dev.net->name);
			}
		}
		pdp_tx_flag = 1;
		if (pdp_tx_flag != 1) {
			pr_err("[MULTIPDP] <%s> pdp_tx_flag set error!\n",
				__func__);
			pdp_tx_flag = 1;
		} else {
			pr_info("[MULTIPDP] <%s> pdp_tx_flag set 1\n", __func__);
		}
